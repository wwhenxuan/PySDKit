...
class STL(object):
    ...
    def _is_odd_int(self, x: int) -> bool:
    ...
    def __call__(self, endog: np.ndarray,
                inner_iter: Optional[int] = None, 
                outer_iter: Optional[int] = None) -> STLResult:
        """
    ...
    def _inner_loop(self):
        ...
        self.seasonal_arr = seasonal_temp[self.period:self.period+self.nobs] - low_pass
    ...
    def _seasonal_smoothing(self, detrended: np.ndarray) -> np.ndarray:
        ...
        for j in range(self.period):
            ...
            start_idx = j
            for i in range(-1, n_sub + 1):  # Include left boundary(-1) and right boundary(n_sub)
    ...
    def _loess(self, x: np.ndarray, y: np.ndarray, window_size: int, degree: int, 
               jump: int = 1, weights: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def _loess_point(self, x: np.ndarray, y: np.ndarray, idx: int, 
                     window_size: int, degree: int, weights: np.ndarray) -> float:
        """
    ...
    def fit_transform(self, endog: np.ndarray,
                     inner_iter: Optional[int] = None, 
                     outer_iter: Optional[int] = None) -> STLResult:
        """
        ...
        if outer_iter is None:
            outer_iter = 10 if self.robust else 0  # 10 outer for robust, 0 for non-robust
