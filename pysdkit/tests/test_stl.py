...
class TestSTL(unittest.TestCase):
    ...
    def test_robust_mode(self) -> None:
        ...
        self.assertTrue(np.all(resid_robust < resid_normal), "稳健模式未正确处理异常值")
    ...
    def test_seasonal_component(self) -> None:
        ...
        # 峰值应出现在周期点
        self.assertGreater(autocorr[0], autocorr[1], "季节性分量自相关峰值错误")
        self.assertGreater(
            autocorr[self.period], autocorr[self.period - 1], "季节性分量周期特性不明显"
        )
    ...
    def test_trend_component(self) -> None:
        ...
        self.assertLess(np.std(trend_diff), np.std(data_diff), "趋势分量不够平滑")
    ...
    def test_different_iterations(self) -> None:
        ...
        # 更多迭代应产生更小的残差
        self.assertLess(std_inner, std_base * 1.05, "增加内迭代未改善分解效果")
        self.assertLess(std_robust, std_base * 1.05, "增加外迭代未改善分解效果")
